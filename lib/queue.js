...
var defaultSetImmediate = (typeof setImmediate === 'function') ? setImmediate : function (fn) {
...
function Queue(process, opts) {
  ...
  self.setImmediate = opts.setImmediate || defaultSetImmediate;
  self.id = opts.id || 'id';
...
Queue.prototype.use = function (store, opts) {
  var self = this;
  var instantiateStore = function (name, options) {
    ...
    try {
      var ExternalStore = require('better-queue-' + normalized);
      return new ExternalStore(options);
    } catch (e) {
      throw new Error('Attempting to require better-queue-' + normalized + ', but failed.\nPlease ensure you have this store installed via npm install --save better-queue-' + normalized);
    }
  };
  if (typeof store === 'string') {
    self._store = instantiateStore(store, opts);
  } else if (typeof store === 'object' && typeof store.type === 'string') {
    self._store = instantiateStore(store.type, store);
